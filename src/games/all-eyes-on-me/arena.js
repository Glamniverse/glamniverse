...
export function createArena(scene){
  ...
  return {group,reset(){portal.scale.setScalar(1)},update(t,phase){portal.scale.setScalar(1+(phase==='playing'?0.018:0.008)*Math.sin(t*1.3))},dispose(){disposed=true;if(!material.map)texture?.dispose()},stats:()=>({panoramaStatus:status})}
}
